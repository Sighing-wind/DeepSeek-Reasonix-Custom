...
1. 到 [Releases](../../releases) 下载 **`Reasonix-Custom-Trial.zip`**（**注意：不要点页面绿色 Code 按钮的 Source code，那是源码**）；
2. 解压到任意文件夹；
3. 双击 **`launcher-custom.cmd`**，首次运行按提示粘贴 DeepSeek 试用 key 并回车，自动完成配置并启动；
4. 首次运行会自动在桌面创建 **Reasonix Custom** 快捷方式，之后点它即可。
...
2. 双击 **`launcher-custom.cmd`**，按提示**粘贴 key 后回车**（无需编辑文件）；
...
把 key 填进 `%APPDATA%\reasonix-custom\.env`（`DEEPSEEK_API_KEY=sk-...` 格式），或在应用内设置页配置。

## 与官方版共存 / 数据隔离

- 定制版使用独立的 `REASONIX_HOME`（`%APPDATA%\reasonix-custom`），**不读写官方版数据**，两者可同时安装互不干扰；
- 卸载：删除程序文件夹和 `%APPDATA%\reasonix-custom` 目录即可，无残留服务。
